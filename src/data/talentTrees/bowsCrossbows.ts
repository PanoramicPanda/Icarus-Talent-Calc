import {FullTrack} from "../../constants/treeStructures.ts";
import {defineTalentTree} from "../../utils/defineTalentTree.ts";

export const bowsCrossbowsTree = {
    talents: defineTalentTree("Bows/Crossbows", [
        {
            name: "Bullseye",
            description: "Increased accuracy with bows",
            rank: 2,
            prerequisites: ["Swift Flight"],
            benefits: [
                [{value: 15, desc: "+{0}% Accuracy with Bows/Crossbows", category: "Ranged_Weapon"}],
                [{value: 30, desc: "+{0}% Accuracy with Bows/Crossbows", category: "Ranged_Weapon"}],
            ],
            position: [2, 3],
            imageName: "Bows/Talent_Bow_IncreasedBowAccuracy",
        },
        {
            name: "Trained Bowman",
            description: "Reduced bow durability loss",
            rank: 2,
            prerequisites: ["Bow Quick Draw"],
            benefits: [
                [{value: -10, desc: "+{0}% Wear Rate of Bows/Crossbows", category: "Item"}],
                [{value: -20, desc: "+{0}% Wear Rate of Bows/Crossbows", category: "Item"}],
                [{value: -30, desc: "+{0}% Wear Rate of Bows/Crossbows", category: "Item"}],
            ],
            position: [2, 2],
            imageName: "Bows/Talent_Bow_TrainedBowman",
        },
        {
            name: "Quick Load",
            description: "Faster bow reload",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 5, desc: "+{0}% Reload Speed of Bows/Crossbows", category: "Ranged_Weapon"}],
                [{value: 10, desc: "+{0}% Reload Speed of Bows/Crossbows", category: "Ranged_Weapon"}],
                [{value: 15, desc: "+{0}% Reload Speed of Bows/Crossbows", category: "Ranged_Weapon"}],
            ],
            position: [0, 3],
            imageName: "Bows/Talent_Bow_QuickReload",
        },
        {
            name: "Easy Draw",
            description: "Reduced stamina usage when drawing bows",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: -5, desc: "+{0}% Stamina Consumed by Bow Actions", category: "Stamina"}],
                [{value: -10, desc: "+{0}% Stamina Consumed by Bow Actions", category: "Stamina"}],
                [{value: -15, desc: "+{0}% Stamina Consumed by Bow Actions", category: "Stamina"}],
            ],
            position: [0, 2],
            imageName: "Bows/Talent_Bow_EasyDraw",
        },
        {
            name: "Bow Quick Draw",
            description: "Increased aim speed with bows",
            rank: 1,
            prerequisites: ["Easy Draw", "More For Less I"],
            benefits: [
                [{value: 5, desc: "+{0}% Aim Speed with Bows/Crossbows", category: "Ranged_Weapon"}],
                [{value: 10, desc: "+{0}% Aim Speed with Bows/Crossbows", category: "Ranged_Weapon"}],
                [{value: 15, desc: "+{0}% Aim Speed with Bows/Crossbows", category: "Ranged_Weapon"}],
            ],
            position: [1, 2],
            imageName: "Bows/Talent_Bow_OverThoseMountains",
        },
        {
            name: "Homing Instinct",
            description: "Chance for arrows to find targets more easily",
            rank: 4,
            prerequisites: ["Soft Spot II"],
            benefits: [
                [{value: 1, desc: "Fired Arrows will lock on to Targets", category: "Ranged_Weapon"}, {value: 1, desc: "Fired Crossbow Bolts will lock on to Targets", category: "Ranged_Weapon"}],
            ],
            position: [7, 2],
            imageName: "Bows/Talent_Bow_HomingArrows",
        },
        {
            name: "Twice the Fun",
            description: "Chance to fire two arrows",
            rank: 3,
            prerequisites: ["Soft Spot I", "Bullseye"],
            benefits: [
                [{value: 5, desc: "{0}% chance to fire an additional Arrow/Bolt", category: "Ranged_Weapon"}],
                [{value: 15, desc: "{0}% chance to fire an additional Arrow/Bolt", category: "Ranged_Weapon"}],
            ],
            position: [5, 3],
            imageName: "Bows/Talent_Bow_TwiceTheFun",
        },
        {
            name: "Hindering Shot",
            description: "Arrow strikes slow your target",
            rank: 4,
            prerequisites: ["Trained Bowsmith", "Soft Spot I"],
            benefits: [
                [{value: 5, desc: "{0}% chance to Slow on Hit with Bows/Crossbows", category: "Modifiers"}],
                [{value: 10, desc: "{0}% chance to Slow on Hit with Bows/Crossbows", category: "Modifiers"}],
                [{value: 15, desc: "{0}% chance to Slow on Hit with Bows/Crossbows", category: "Modifiers"}],
            ],
            position: [5, 1],
            imageName: "Bows/Talent_Bow_HinderingShot",
        },
        {
            name: "Pinning Shot",
            description: "Chance for arrow strikes to immobilize target",
            rank: 4,
            prerequisites: ["Soft Spot II"],
            benefits: [
                [{value: 5, desc: "{0}% chance to Immobilize on Hit with Bows/Crossbows", category: "Modifiers"}],
            ],
            position: [7, 3],
            imageName: "Bows/Talent_Bow_PinningShot",
        },
        {
            name: "Soft Spot I",
            description: "Increased critical hit multiplier on bows",
            rank: 3,
            prerequisites: ["Bullseye", "Trained Bowsmith", "Trained Bowman"],
            benefits: [
                [{value: 5, desc: "+{0}% Critical Damage with Bows/Crossbows", category: "Generic_Damage"}],
                [{value: 15, desc: "+{0}% Critical Damage with Bows/Crossbows", category: "Generic_Damage"}],
            ],
            position: [4, 2],
            imageName: "Bows/Talent_Bow_SoftSpot1",
        },
        {
            name: "Soft Spot II",
            description: "Increased critical hit multiplier on bows",
            rank: 3,
            prerequisites: ["Twice the Fun"],
            benefits: [
                [{value: 10, desc: "+{0}% Critical Damage with Bows/Crossbows", category: "Generic_Damage"}],
                [{value: 25, desc: "+{0}% Critical Damage with Bows/Crossbows", category: "Generic_Damage"}],
            ],
            position: [6, 3],
            imageName: "Bows/Talent_Bow_SoftSpot2",
        },
        {
            name: "Trained Bowsmith",
            description: "Crafted bows have increased durability",
            rank: 2,
            prerequisites: ["Increased Arrow Damage I", "Bow Quick Draw", "More For Less I"],
            benefits: [
                [{value: 1, desc: "Alters Crafted Bows to Provide a Bonus to Health", category: "Misc"}],
            ],
            position: [2, 1],
            imageName: "Bows/Talent_Bow_CraftyFletcher1",
        },
        {
            name: "Crafty Fletcher",
            description: "Chance to craft double when crafting arrows",
            rank: 3,
            prerequisites: ["Trained Bowsmith"],
            benefits: [
                [{value: 10, desc: "{0}% chance to Craft an additional Arrow/Bolt", category: "Crafting"}],
            ],
            position: [3, 0],
            imageName: "Bows/Talent_Bow_CraftyFletcher2",
        },
        {
            name: "Bow Agility",
            description: "Faster movespeed while holding bows",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 5, desc: "+{0}% Movement Speed with a Bow/Crossbow Equipped", category: "Movement"}],
                [{value: 8, desc: "+{0}% Movement Speed with a Bow/Crossbow Equipped", category: "Movement"}],
                [{value: 10, desc: "+{0}% Movement Speed with a Bow/Crossbow Equipped", category: "Movement"}],
            ],
            position: [0, 0],
            imageName: "Bows/Talent_Bow_BowAgility",
        },
        {
            name: "Swift Flight",
            description: "Increased arrow speed",
            rank: 1,
            prerequisites: ["Quick Load"],
            benefits: [
                [{value: 3, desc: "+{0}% Arrow Speed", category: "Misc"}],
                [{value: 5, desc: "+{0}% Arrow Speed", category: "Misc"}],
                [{value: 10, desc: "+{0}% Arrow Speed", category: "Misc"}],
            ],
            position: [1, 3],
            imageName: "Bows/Talent_Bow_SwiftFlight",
        },
        {
            name: "Increased Arrow Damage I",
            description: "Adds a bonus to arrow damage",
            rank: 1,
            prerequisites: ["Bow Agility", "More For Less I"],
            benefits: [
                [{value: 2, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged_Weapon"}],
                [{value: 5, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged_Weapon"}],
                [{value: 10, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged_Weapon"}],
            ],
            position: [1, 0],
            imageName: "Bows/Talent_Bow_IncreasedArrowDamage1",
        },
        {
            name: "Increased Arrow Damage II",
            description: "Adds a further bonus to arrow damage",
            rank: 2,
            prerequisites: ["Increased Arrow Damage I"],
            benefits: [
                [{value: 2, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged_Weapon"}],
                [{value: 5, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged_Weapon"}],
                [{value: 10, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged_Weapon"}],
            ],
            position: [2, 0],
            imageName: "Bows/Talent_Bow_IncreasedArrowDamage2",
        },
        {
            name: "More For Less II",
            description: "Cheaper bow crafting",
            rank: 3,
            prerequisites: ["Trained Bowsmith"],
            benefits: [
                [{value: -25, desc: "+{0}% Resource Cost of Crafted Bows/Crossbows", category: "Crafting"}],
            ],
            position: [4, 0],
            imageName: "Bows/Talent_Bow_MoreForLess2",
        },
        {
            name: "More For Less I",
            description: "Cheaper bow crafting",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: -25, desc: "+{0}% Resource Cost of Crafted Bows/Crossbows", category: "Crafting"}],
            ],
            position: [0, 1],
            imageName: "Bows/Talent_Bow_MoreForLess",
        },
        {
            name: "Wounding Shot",
            description: "Arrow strikes bleed your target over time",
            rank: 4,
            prerequisites: ["Hindering Shot"],
            benefits: [
                [{value: 5, desc: "{0}% chance to Wound on Hit with Bows/Crossbows", category: "Modifiers"}],
                [{value: 10, desc: "{0}% chance to Wound on Hit with Bows/Crossbows", category: "Modifiers"}],
                [{value: 15, desc: "{0}% chance to Wound on Hit with Bows/Crossbows", category: "Modifiers"}],
            ],
            position: [7, 1],
            imageName: "Bows/Talent_Bow_WoundingShot",
        },
        {
            name: "Supply and Demand",
            description: "Increased damage from crafted arrows or bolts",
            rank: 4,
            prerequisites: ["Hindering Shot"],
            benefits: [
                [{value: 1, desc: "No description", category: "Misc"}],
                [{value: 2, desc: "No description", category: "Misc"}],
                [{value: 3, desc: "No description", category: "Misc"}],
            ],
            position: [7, 0],
            imageName: "Bows/Talent_Bow_SupplyDemand",
        },
    ]),
    fullTracks: [] as FullTrack[]
};
